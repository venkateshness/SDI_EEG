...
def SDI(graph, condition):
    ...
    for band in ['theta', 'alpha', 'low_beta', 'high_beta', 'gamma', 'widerband']:
        ...
        for sub, signal in tqdm(envelope_signal_bandpassed.items()):
            ...
            lf_comp, hf_comp = utility_functions.fullpipeline_time_varying_SDI(envelope=signal, eigevecs=eigenvectors, eigvals=eigenvals)
            ...
            for surr in range(np.shape(surr_signal)[0]):
                ...
                lf, hf = utility_functions.fullpipeline_time_varying_SDI(graph_signal, eigenvectors, eigenvals, is_surrogate=True)
                SDI = hf/lf
                surr_SDI.append(SDI)
    ...
    np.savez_compressed(f"{HOMEDIR}/revision/Generated_data_revision/{condition}/Graph_SDI_related/time_varying_SDI/empirical_SDI.npz", **empirical_SDI_bundle)
    np.savez_compressed(f"{HOMEDIR}/revision/Generated_data_revision/{condition}/Graph_SDI_related/time_varying_SDI/surrogate_SDI.npz", **surrogate_SDI_bundle)
#%%
#movie
