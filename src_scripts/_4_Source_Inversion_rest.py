...
def noise_covariance(subject):
    ...
    cov = mne.EpochsArray(
        epochs_resampled["30"][0].get_data()[:, :, rand],
...
subjects = sorted(list(os.listdir(f'{HOMEDIR}/revision/Generated_data_revision/rest/preprocessed_dataset/')))

n_chans_after_preprocessing = 91
time_in_samples = 20*fs
stc_bundle = dict()
...
for id in range(len(subjects)): 
    ...
    data_rest, events_list = (
        mne.io.read_raw_fif(
            f"{HOMEDIR}/revision/Generated_data_revision/rest/preprocessed_dataset/{subjects[id]}/raw.fif",
            verbose=False,
        ),
        np.load(
            f"{HOMEDIR}/revision/Generated_data_revision/rest/preprocessed_dataset/{subjects[id]}/events.npz"
        )["resting_state_events"],
    )

    info_d = mne.create_info(
        data_rest.info["ch_names"][:-1], sfreq=125, ch_types="eeg", verbose=False
    )
    data_rest_sliced = mne.io.RawArray(data_rest.get_data()[:-1, :], info_d, verbose=False)
    ...
    the_epoch = mne.EpochsArray(
        np.expand_dims(np.mean(sliced_epoch.get_data(), axis=0)[:, :10000], axis=0),
        mne.create_info(
            data_rest_sliced.info["ch_names"], sfreq=500, ch_types="eeg", verbose=False
        ),
    ).resample(125)
    ...
    if id == 0:  # Reusing the fwd_model; cut down some time
        ...
        fwd_model = forward_model(
            data_rest, trans=trans, source_space=source_space, bem=bem
        )
    ...
    del raw, the_epoch, data_rest, events_list, sliced_epoch, info_d, eloreta_activation, inverse_operator
...
np.savez_compressed(f"{HOMEDIR}/revision/Generated_data_revision/rest/cortical_surface_related/parcellated_widerband.npz", **parcellated)
...
for band, (low, high) in band_ranges.items():
    ...
    np.savez_compressed(f'{HOMEDIR}/revision/Generated_data_revision/rest/cortical_surface_related/{band}_bandpassed', **band_data)
