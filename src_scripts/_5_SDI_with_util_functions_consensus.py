...
def SDI(graph, condition):
    consensus_graph = np.mean(list(graph.values()), axis = 0)
    ...
    for band in ['theta', 'alpha', 'low_beta', 'high_beta', 'gamma', 'wideband']:
        ...
        envelope_signal_bandpassed = np.load(f"{HOMEDIR}/Generated_data/{condition[:-10]}/cortical_surface_related/{band}_bandpassed.npz")
        s_bundle = list()
        e_bundle = list()

        for sub, signal in tqdm(envelope_signal_bandpassed.items()):
            _, eigenvals, eigenvectors = utility_functions.eigmodes(W=consensus_graph)
